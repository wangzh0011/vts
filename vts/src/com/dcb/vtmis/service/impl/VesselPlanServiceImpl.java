...
public class VesselPlanServiceImpl implements VesselPlanService{
	...
	public Map<String,VesselPlan> applyVesselPlan(String token) {//申报大船
		...
		//申报入口
		doApplyVesselPlan(list, token, "大船");
		return map;
	}
			...
			if(i < list.size()-1){
				if(list.get(i).getEnShipName().equals(list.get(i+1).getEnShipName()) 
					...
					/*if(list.get(i).getPlanTime().equals(list.get(i+1).getPlanTime())){//判断预计靠离泊时间是否一致
						...
						//mailUtil.sendEmail(to, "发现新大陆！！！", htmlFactory.Make_Notice(list.get(i).getReferenceId()+","+list.get(i+1).getReferenceId(),list.get(i).getEnShipName()+","+list.get(i+1).getEnShipName(),list.get(i).getShipNo(),"",""), mailSender);
						if(list.get(i).getVesselActionCode().equals(1)){//靠泊
							if(Utils.isNotNull(list.get(i).getEst_discharge()) /*&& Utils.isNotNull(list.get(i+1).getEst_load())*/){
								log.info("保留"+list.get(i).getReferenceId());
								log.info("移除"+list.get(i+1).getReferenceId());
								list.remove(i+1);	
							}else if(Utils.isNotNull(list.get(i+1).getEst_discharge()) /*&& Utils.isNotNull(list.get(i).getEst_load())*/){
								log.info("保留"+list.get(i+1).getReferenceId());
								log.info("移除"+list.get(i).getReferenceId());
								list.remove(i);
							}else{//随机移除
								log.info("保留"+list.get(i).getReferenceId());
								log.info("移除"+list.get(i+1).getReferenceId());
...
//					}
		...
		//申报入口
		doApplyVesselPlan(list, token, "驳船");		
		return map;
	}
...
//			mailUtil.sendEmail(to, Constants._200, htmlFactory.Make_Notice(vp.getReferenceId(),vp.getEnShipName(),vp.getShipNo(),Constants._200), mailSender);
			...
			//发送邮件
			mailUtil.sendEmail(to, Constants._400, htmlFactory.Make_Notice(vp.getReferenceId(),vp.getEnShipName(),vp.getShipNo(),Constants._400,"，请检查船舶计划数据格式，系统将在30分钟后重新申报此船舶计划。"), mailSender);
			log.info("发送邮件成功");
			log.info("========================================================================================");
			log.info("========================================================================================");
		}
			...
			mailUtil.sendEmail(to, Constants._401, htmlFactory.Make_Notice(vp.getReferenceId(),vp.getEnShipName(),vp.getShipNo(),Constants._401,"，请联系海事局检查账号权限问题。"), mailSender);
		}
